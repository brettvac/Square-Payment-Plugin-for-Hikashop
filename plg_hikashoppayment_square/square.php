...
class plgHikashoppaymentSquare extends hikashopPaymentPlugin
{
    ...
    public function onPaymentNotification(&$statuses)
    {
        ...
        if (!isset($payload['data']['object']['payment']['location_id'])) {
            http_response_code(400);
            exit('Invalid webhook payload structure or missing location_id');
        }

        $location_id = $payload['data']['object']['payment']['location_id'];

        // Load the correct payment parameters BEFORE doing anything else
        $db = Factory::getDbo();
        ...
        if (!$methodFound || empty($this->payment_params)) {
            $this->logSquareDebug("Webhook Error: Could not find published Square payment method for location ID $location_id");
            http_response_code(400);
            exit('Unknown location ID');
        }
        ...
        if (!isset($payload['type']) || !in_array($payload['type'], ['payment.updated', 'payment.created'])) {
            http_response_code(200);
            exit('Ignored event type'); // Respond 200 OK for irrelevant webhooks
        }

        if (!isset($payload['data']['object']['payment'])) {
            http_response_code(400);
            exit('Invalid webhook payload structure');
        }
        ...
        if (empty($squareOrder) || empty($squareOrder['reference_id'])) {
            ...
            exit('Square Order not found or no reference_id');
        }
    ...
    private function logSquareDebug($message)
    {
        if ($this->payment_params->debug) {
            ...
            // Add the logger category if it hasn't been initialized
            Log::addLogger(['text_file' => 'square_payment.php'], Log::ALL, ['square']);
            Log::add($message, Log::DEBUG, 'square');
            // Write to the native HikaShop log as well
